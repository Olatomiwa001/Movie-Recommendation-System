...
def add_movie(conn, title, genre, year=None, poster_url='', popularity=0):
    ...
    try:
        ...
        if existing:
            logging.info(f"Movie '{title}' already exists with ID {existing[0]}")
            return existing[0]
        ...
        conn.commit()
        logging.info(f"Added movie '{title}' with ID {cursor.lastrowid}")
...
def bulk_import_movies(conn, csv_text):
    ...
    try:
        ...
        print(f"Error importing movies from CSV: {e}")
        return False
...
def visualize_recommendations(user_id, recommendations):
    ...
    if not recommendations or not isinstance(recommendations, list):
        print("No recommendations to visualize or invalid data format")
        return False
    ...
    if 'title' not in rec_df.columns or 'score' not in rec_df.columns:
        print("Recommendations data is missing required columns ('title', 'score')")
        return False
